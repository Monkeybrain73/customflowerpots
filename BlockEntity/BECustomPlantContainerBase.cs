...
{
    public abstract class BlockEntityCustomPlantContainerBase : BlockEntityContainer, ITexPositionSource, IRotatable
    {
        ...
        protected MeshData potMesh;
        protected MeshData[] contentMeshes;
        protected RoomRegistry roomReg;

        protected ICoreClientAPI capi;
        protected ITexPositionSource contentTexSource;
        protected PlantContainerProps curContProps;
        protected Dictionary<string, AssetLocation> shapeTextures;

        protected bool hasSoil => inv.Any(slot => !slot.Empty);

        public abstract int SlotCount { get; }
        protected abstract Vec3f SlotIndexToOffset(int i);
        protected abstract int GetSlotFromClick(BlockSelection blockSel);

        public BlockEntityCustomPlantContainerBase()
        {
            inv = new InventoryGeneric(SlotCount, null, null, null);
            inv.OnAcquireTransitionSpeed += (transType, stack, mulByConfig) => 0;
        }

        protected override void OnTick(float dt) { /* no ticking */ }
        ...
        private MeshData[] GenContentMeshes(ITesselatorAPI tesselator, ItemStack content)
        {
            ...
            {
                assets = new List<IAsset> { Api.Assets.TryGet(compoShape.Base.WithPathPrefixOnce("shapes/").WithPathAppendixOnce(".json")) };
            }
