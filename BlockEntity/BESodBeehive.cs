...
{
    internal class BESodBeehive : BlockEntityBeehive,IAnimalFoodSource
    {
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            isWildHive = Block.FirstCodePart() != "sodroofskep";
            if (!isWildHive && api.Side == EnumAppSide.Client)
            {
                ICoreClientAPI capi = api as ICoreClientAPI;

                if (Block.FirstCodePart() == "sodroofskep")
                {
                    Block fullSkep = api.World.GetBlock(new AssetLocation("apeflowerpots:sodroofskep-populated-free-" + orientation));

                    MeshData mesh;
                    capi.Tesselator.TesselateShape(
                        fullSkep,
                        Shape.TryGet(api, "apeflowerpots:shapes/block/misc/sodskep/skep-harvestable-free.json"),
                        out mesh,
                ...
                {
                    Block fullSkep = api.World.GetBlock(new AssetLocation("game:skep-populated-east"));

                    MeshData mesh;
                    capi.Tesselator.TesselateShape(
                        fullSkep,
                        Shape.TryGet(api, "game:shapes/block/beehive/skep-harvestable.json"),
                        out mesh,
                        new Vec3f(0, BlockFacing.FromCode(orientation).HorizontalAngleIndex * 90 - 90, 0)
                    );
                    api.ObjectCache["beehive-harvestablemesh-" + orientation] = mesh;
        ...
        private void OnScanForEmptySkep(float dt)
        {
            ...
            Api.World.BlockAccessor.WalkBlocks(Pos.AddCopy(minX, -5, minZ), Pos.AddCopy(minX + size - 1, 5, minZ + size - 1), (block, x, y, z) =>
            {
                ...
                if (blockcode.StartsWithOrdinal("sodroofskep-empty")) scanEmptySkeps.Add(new BlockPos(x, y, z));
                else if (blockcode.StartsWithOrdinal("sodroofskep-populated") || blockcode.StartsWithOrdinal("skep-populated") || blockcode.StartsWithOrdinal("wildbeehive")) scanQuantityNearbyHives++;
                Api.World.Logger.Event($"Scanning for skeps-> Name: {blockcode}");
            });

            scanIteration++;
            Api.World.Logger.Event($"Number of live skeps found: {scanQuantityNearbyHives}");
            Api.World.Logger.Event("Number of scans done: {0}", scanIteration);
        ...
        private void OnScanComplete()
        {
            ...
            if (skepToPop != null)
            {
                ...
                popHiveAfterHours = (float)(0.75 * popHiveAfterHours + 0.25 * newPopHours);
        ...
        private void TryPopCurrentSkep()
        {
            ...
            if (skepToPopBlock == null || !(skepToPopBlock is BlockSodSkep) || !(skepToPopBlock is BlockSkep))
            {
            ...
            string bname = skepToPopBlock.Code.Path;

            if (bname.StartsWithOrdinal("sodroofskep-empty"))
            {
                string blockcode = "apeflowerpots:sodroofskep-populated-free-" + orient;
                Block fullSkep = Api.World.GetBlock(new AssetLocation(blockcode));
                Api.World.Logger.Notification("Tried to populate: {0}", blockcode);
                Api.World.Logger.Notification("Full name: {0}", bname);

                if (fullSkep == null)
                {
                    Api.World.Logger.Warning("BESodBeehive.TryPopSkep() - block with code {0} does not exist?", blockcode);
                    return;
                }
            ...
            if (bname.StartsWithOrdinal("skep-empty"))
            {
                string blockcode = "game:skep-populated-free-" + orient;
                Block fullSkep = Api.World.GetBlock(new AssetLocation(blockcode));
                Api.World.Logger.Notification("Tried to populate: {0}", blockcode);
                Api.World.Logger.Notification("Full name: {0}", bname);
